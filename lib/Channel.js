...
class Channel extends EnhancedEventEmitter_1.default {
    /**
     * @private
     */
    constructor({ producerSocket, consumerSocket, pid }) {
        super(new Logger_1.default(`Channel[pid:${pid}]`));
        this._closed = false;
        this._nextId = 0;
        this._sents = new Map();
        this._logger.debug('constructor()');
        this._workerLogger = new Logger_1.default(`worker[pid:${pid}]`);
            ...
            while (true) // eslint-disable-line no-constant-condition
             {
                ...
                try {
                    ...
                    this._logger.error('invalid netstring data received from the worker process: %s', String(error));
                    // Reset the buffer and exit.
                ...
                try {
                    ...
                    this._logger.error('received invalid message from the worker process: %s', String(error));
                }
    ...
    _processMessage(msg) {
        ...
        if (msg.id) {
            ...
            if (!sent) {
                this._logger.error('received response does not match any sent request [id:%s]', msg.id);
                return;
            }
            if (msg.accepted) {
                this._logger.debug('request succeeded [method:%s, id:%s]', sent.method, sent.id);
                sent.resolve(msg.data);
            }
            else if (msg.error) {
                this._logger.warn('request failed [method:%s, id:%s]: %s', sent.method, sent.id, msg.reason);
                switch (msg.error) {
                    case 'TypeError':
                ...
                this._logger.error('received response is not accepted nor rejected [method:%s, id:%s]', sent.method, sent.id);
            }
        }
        ...
        // Otherwise unexpected message.
            ...
            this._logger.error('received message is not a response nor a notification');
        }
    }
}
